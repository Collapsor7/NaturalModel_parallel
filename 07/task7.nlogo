...
to setup
  ...
  ask nodes [
    create-links-with other nodes[
      set color blue
      set ph 1
      set len max (list 0.01 (link-length / max-pxcor))
    ]
...
to setup-node
  ...
  if layout = "figure 2"
  [
    let n-size graph-size / 2
    let r max-pxcor - 1
    let a (360 / n-size) * (who mod n-size)
  ...
  [
    ...
    if who < n-size
    [ set r r * 0.8
      set a a + (360 / n-size) / 2
    ]
    setxy ( r * sin  a ) (r * cos a )
  ]
  if layout = "figure 4"[

    let n-size graph-size / 3
    let r max-pxcor - 1
    let a (360 / n-size) * (who mod n-size)
    ...
    if n-size <= who and who < 2 * n-size
    [
      set r r * 0.5
    ]
    setxy ( r * sin  a ) (r * cos a )
  ...
  [
    let n ceiling sqrt graph-size
    let d (2 * max-pxcor - 2) / (n - 1)
    let new-d (2 * max-pxcor - 2) / n
    set xcor min-pxcor + 1 + d * (who mod n)
    ifelse (who mod n) mod 2 = 0 [
      set ycor min-pycor + 1 + new-d * floor ( who / n)
    ][
      set ycor min-pycor + 1 + new-d  * floor ( who / n) + 0.5 * new-d
    ]
  ]
end
...
to go
  ...
  repeat graph-size [ask ants [ move-ants]]
  ask ants [
...
to-report choose-next
  ifelse empty? to-visit [report 0]
  [
    let weights map get-weight to-visit
    let total sum weights
    set rnd random-float total
    set sel 0

    let index 0
    while [index < length to-visit] [
      let c item index to-visit
...
layout
layout
"random" "circle" "grid" "figure 1" "figure 2" "figure 3" "figure 4" "figure 5"
7
