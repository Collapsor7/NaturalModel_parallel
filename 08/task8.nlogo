...
globals [free-patches target-patch]
...
to setup-target
  ...
  if any? free-patches [
    set target-patch one-of free-patches
    ask target-patch [
      sprout-targets 1[
        set color white
        set shape "x"
...
to diffusion
  ...
  while [minv < 10 ^ -7][
    ...
    set minv min [p-val] of patches with [free?]
    set maxv max [p-val] of patches with [free?]
...
to setup-colony
  ask colonies [die]
  let center one-of free-patches
  ask center [
    sprout-colonies colony-size [
      set shape "circle"
      set size 0.8
      move-to one-of patches in-radius 5 with [ free? ]
      set val eval-me
    ]
  ]
  reset-ticks
...
to-report eval-me
  ifelse swarm?[
    let a mean [h distance myself] of colonies
    report [p-val] of patch-here + 0.0001 * a
  ][
...
to compete
  let opponent one-of other colonies
  if opponent = nobody [stop]
  let cur-val val
  ifelse [val] of opponent > cur-val [
    die
  ][
    ask opponent [die]
    hatch 1 [
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count colonies"
