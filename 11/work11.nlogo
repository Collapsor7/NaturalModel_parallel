...
to make-rules
  ...
  if m  = 2 [
    if r = 1 [
      ;set number random (2 ^ 8)
      set repeat-n 8
    ]
    if r = 2 [
      set number random (2 ^ 32)
      set repeat-n 32
    ]
  ]
  if m = 3 [
...
to setup-patch
  ifelse pycor != max-pycor [set state -1]
  [
    if init-state = "single 1" [
      ...
      if m = 3 [
        ifelse pxcor = 0
        [set state one-of [1 2] ]
        [set state 0]
      ]

    ]
    if init-state = "random" [
      if m = 2 [ set state one-of [0 1] ]
      if m = 3 [ set state one-of [0 1 2] ]
    ]
  ]
  set pre-state state
  recolor
...
to update-patch
  ...
  [
    ...
    [
      ...
      set k (m ^ 4) * a + (m ^ 3) * b + ( m ^ 2) * c + m * d + f
    ]
    set p item k rules
  ]
  ifelse reversible? [
    set state (p + pre-state ) mod m
  ][ set state p]
  recolor
  set pre-state state
end
